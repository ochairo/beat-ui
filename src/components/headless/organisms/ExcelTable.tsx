...
export type ExcelTableCellType =
  | "checkbox"
  | "date"
  | "link"
  | "number"
...
export type ExcelTableCellValue = string | ReturnType<typeof Decimal>;
...
export interface ExcelTableCell {
  ...
  readonly value: ExcelTableCellValue;
}

export type ExcelTableRow = readonly ExcelTableCell[];

export type ExcelTableGrid = readonly ExcelTableRow[];

export interface ExcelTableColumn {
  readonly align?: "center" | "left" | "right";
  readonly cellOptions?: readonly string[];
  readonly cellType?: ExcelTableCellType;
...
export interface ExcelTableSelection {
...
export interface ExcelTableCellPresentationState {
...
export interface ExcelTableStyles {
  readonly cellButton?:
    | ((state: ExcelTableCellPresentationState) => string)
    | undefined;
...
export interface ExcelTableProps extends BeatUiAccessibilityProps {
  readonly columns: readonly ExcelTableColumn[];
  readonly createColumn?: (columnIndex: number) => ExcelTableColumn;
  readonly defaultSort?: BeatUiTableSort;
  readonly defaultValue?: ExcelTableGrid;
  readonly onColumnsChange?: (columns: readonly ExcelTableColumn[]) => void;
  readonly onValueChange?: (grid: ExcelTableGrid) => void;
  readonly rowCount?: number;
  readonly showRowHeaders?: boolean;
  readonly showStructureControls?: boolean;
  readonly styles?: ExcelTableStyles;
  readonly value?: BeatUiState<ExcelTableGrid>;
}

// ─── Utility functions ────────────────────────────────────────────────────────

function createEmptyCell(): ExcelTableCell {
  return { value: "" };
}

function createEmptyRow(columnCount: number): ExcelTableRow {
  return Array.from({ length: columnCount }, () => createEmptyCell());
}

function normalizeCell(cell: ExcelTableCell | undefined): ExcelTableCell {
  return cell ?? createEmptyCell();
}

function normalizeGrid(
  grid: ExcelTableGrid | undefined,
  columnCount: number,
  rowCount: number,
): ExcelTableGrid {
...
export function formatCellValue(value: ExcelTableCellValue): string {
...
function parseClipboardCellValue(value: string): ExcelTableCellValue {
...
function getSelectionBounds(selection: ExcelTableSelection): {
...
function isCellSelected(
  selection: ExcelTableSelection | null,
  rowIndex: number,
  columnIndex: number,
): boolean {
...
interface ExcelTableEditingCell {
  readonly columnIndex: number;
  readonly rowIndex: number;
}

interface ExcelTableEditingTarget extends ExcelTableEditingCell {
  readonly value: string;
}

interface ExcelTableCommitOptions {
  readonly focusContainer?: boolean;
  readonly moveDown?: boolean;
  readonly target?: ExcelTableEditingTarget;
}

interface ExcelTableEditActivationOptions {
  readonly deferWhenSwitching?: boolean;
}

interface ExcelTableCellViewProps {
  readonly align?: "center" | "left" | "right";
  ...
  readonly cellType?: ExcelTableCellType;
  readonly columnIndex: number;
  readonly editingCell: BeatUiState<ExcelTableEditingCell | null>;
  readonly editingValue: BeatUiState<string>;
  readonly editWidgetStyle?: string | undefined;
  readonly onActivateEdit: (
    rowIndex: number,
    columnIndex: number,
    options?: ExcelTableEditActivationOptions,
  ) => void;
  readonly onCancelEdit: () => void;
  readonly onCommitEdit: (options?: ExcelTableCommitOptions) => void;
  ...
  readonly row: BeatUiState<ExcelTableRow>;
  readonly rowIndex: number;
  readonly selection: BeatUiState<ExcelTableSelection | null>;
}

const ExcelTableCellView = component<ExcelTableCellViewProps>((props) => {
  const syncPresentation = (element: HTMLButtonElement): void => {
    ...
    if (props.cellButtonStyle !== undefined) {
      const state: ExcelTableCellPresentationState = {
        isSelected: selected,
        isAnchor: active,
        isEditing: editing,
...
export const ExcelTable = component<ExcelTableProps>((props) => {
  ...
  const state = createControllableState<ExcelTableGrid>({
  ...
  const columnsState = pulse<readonly ExcelTableColumn[]>([...props.columns]);
  const selection = pulse<ExcelTableSelection | null>(
  ...
  const editingCell = pulse<ExcelTableEditingCell | null>(null);
  ...
  const createGeneratedColumn = (columnIndex: number): ExcelTableColumn => {
  ...
  const setColumns = (nextColumns: readonly ExcelTableColumn[]): void => {
  ...
  const setSelectionState = (
    nextSelection: ExcelTableSelection | null,
  ): void => {
  ...
  const handleCommitEdit = (options?: ExcelTableCommitOptions): void => {
    const currentEditingCell = editingCell.get();
    const targetCell =
  ...
  const handleKeyDown = (event: KeyboardEvent): void => {
    ...
    const nonTextTypes: readonly ExcelTableCellType[] = [
      "checkbox",
      "select",
      "radio",
      "date",
